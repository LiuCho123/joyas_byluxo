...
import { RefreshCw, MessageCircle, Heart, Bookmark, Share2, Play, Plus, X, Camera, Video, Trash2, Pencil, PackageSearch, Zap, Search, Store, MessageSquare } from 'lucide-react';
...
const ContentGallery = () => {
    ...
    const handleEliminarVideo = async (id) => {
        ...
        try {
            ...
            else alert("Error al eliminar. Revisa el servidor.");
        } catch (error) {
    ...
    const handleRegistrarVideo = async (e) => {
        ...
        try {
            ...
            console.error("Error al registrar/editar contenido:", error);
        }
    };

    const getEstadoVideo = (pubJoyasRelacion) => {
        if (!pubJoyasRelacion || pubJoyasRelacion.length === 0) return { msg: "VLOG / GENERAL", color: "text-green-400 bg-green-400/10 border-green-500/30" };
    ...
    const generarReporteEstratega = () => {
        ...
        reporteTexto += `(Generado para análisis comparativo de retención y limpieza)\n\n`;
        ...
        <div className="max-w-7xl mx-auto p-6 bg-zinc-950 text-gray-200 rounded-xl shadow-2xl border border-zinc-800 font-sans mt-6">
            <div className="flex justify-between items-center mb-6 pb-6 border-b border-zinc-800">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-zinc-500">Auditoría Visual y Ecosistema de Redes</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    <button onClick={cargarDatos} className="p-2 bg-zinc-900 border border-zinc-800 rounded hover:bg-zinc-800 transition-colors">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-8">
                <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => setActivePlatform('TikTok')} className={`px-6 py-2.5 font-bold tracking-widest rounded-lg flex items-center gap-2 transition-all text-xs ${activePlatform === 'TikTok' ? 'bg-black text-white border border-zinc-700' : 'bg-zinc-900 text-zinc-500 border border-zinc-800 hover:text-white'}`}><Video className="w-4 h-4" /> TK</button>
                    <button onClick={() => setActivePlatform('Marketplace')} className={`px-6 py-2.5 font-bold tracking-widest rounded-lg flex items-center gap-2 transition-all text-xs ${activePlatform === 'Marketplace' ? 'bg-blue-600 text-white' : 'bg-zinc-900 text-zinc-500 border border-zinc-800 hover:text-white'}`}><Store className="w-4 h-4" /> MKP</button>
                    <button onClick={() => setActivePlatform('WhatsApp')} className={`px-6 py-2.5 font-bold tracking-widest rounded-lg flex items-center gap-2 transition-all text-xs ${activePlatform === 'WhatsApp' ? 'bg-green-600 text-white' : 'bg-zinc-900 text-zinc-500 border border-zinc-800 hover:text-white'}`}><MessageSquare className="w-4 h-4" /> WSP</button>
                </div>
                ...
                <div className="flex flex-1 items-center gap-3 w-full xl:w-auto">
                    <div className="flex items-center bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 w-full">
                        ...
                        <input type="text" placeholder="Buscar título o joya..." value={searchTermGallery} onChange={(e) => setSearchTermGallery(e.target.value)} className="bg-transparent border-none outline-none text-sm text-white placeholder-zinc-500 w-full" />
                    </div>
                    <button onClick={generarReporteEstratega} disabled={isGenerating} className="w-full md:w-auto bg-zinc-800 text-zinc-300 font-bold px-6 py-2.5 rounded hover:bg-zinc-700 hover:text-white border border-zinc-700 shadow-lg text-xs tracking-wider">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {pubsVisualizar.length === 0 ? (
                    <div className="col-span-full p-10 text-center text-zinc-500 border border-zinc-800 border-dashed rounded-xl">
                        No hay contenido registrado o ninguno coincide con la búsqueda.
                    </div>
                ) : (
                    pubsVisualizar.map((pub) => {
                        const estado = getEstadoVideo(pub.relaciones);
                        return (
                            <div key={pub.id} className={`bg-zinc-900 border rounded-xl overflow-hidden flex flex-col transition-all hover:border-zinc-500 ${estado.msg.includes('BORRAR') ? 'border-red-900/50' : 'border-zinc-800'}`}>
                                <div className="p-4 border-b border-zinc-800 bg-black/40">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className="flex gap-3">
                                            <button onClick={() => abrirParaEditar(pub)} className="text-zinc-600 hover:text-white"><Pencil className="w-4 h-4" /></button>
                                            <button onClick={() => handleEliminarVideo(pub.id)} className="text-zinc-600 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </div>
                                    <h3 className="font-bold text-sm text-zinc-200 line-clamp-2 min-h-[2.5rem]">{pub.titulo}</h3>
                                </div>
                                <div className="p-4 flex-1 flex flex-col justify-between">
                                    <div className={`text-[10px] px-2 py-1.5 rounded text-center mb-4 uppercase tracking-widest ${estado.color}`}>{estado.msg}</div>
                                    <div className="space-y-1.5">
                                        {(!pub.relaciones || pub.relaciones.length === 0) ? (
                                            <p className="text-xs text-zinc-500 italic text-center">No expone joyas.</p>
                                        ) : (
                                            pub.relaciones.map(rel => {
                                                ...
                                                return (
                                                    <div key={rel.id} className={`text-xs p-2 rounded border flex flex-col gap-1 ${desc ? 'bg-yellow-950/20 border-yellow-700/50 text-yellow-300' : 'bg-zinc-950 border-zinc-800 text-zinc-400'}`}>
                                                        <span className="truncate font-bold">{rel.joya.nombre}</span>
                                                        <div className="flex justify-between text-[10px]">
                                                            <span className="flex gap-1 items-center"><Zap className="w-3 h-3" /> G: {rel.stockAlSubir}</span>
                                                            <span className={`flex gap-1 items-center ${desc ? 'text-yellow-400 font-bold' : ''}`}><PackageSearch className="w-3 h-3" /> A: {stockActual}</span>
                                                        </div>
                                                    </div>
                                                );
                                            })
                                ...
                                {(pub.plataforma === 'Instagram' || pub.plataforma === 'TikTok') && (
                                    <div className="p-3 border-t border-zinc-800 bg-black/40 grid grid-cols-2 gap-y-2 gap-x-2">
                                        <div className="flex items-center gap-1.5 text-zinc-400"><Play className="w-3.5 h-3.5" /><span className="text-xs font-bold">{pub.reproducciones || 0}</span></div>
                                        <div className="flex justify-end gap-1.5 text-zinc-500"><Heart className="w-3.5 h-3.5" /><span className="text-xs">{pub.likes || 0}</span></div>
                                    </div>
                                )}
                            </div>
                        );
                ...
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
                    <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-xl shadow-2xl w-full max-w-2xl">
                        <div className="flex justify-between items-center mb-6 border-b border-zinc-800 pb-4">
                            <h3 className="text-xl font-light tracking-widest text-zinc-100 uppercase">{editingId ? '✏️ Editar Contenido' : '🎬 Anotar Contenido'}</h3>
                            <button onClick={cerrarModal} className="text-zinc-500 hover:text-white"><X className="w-6 h-6" /></button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                            <div className="flex flex-col md:col-span-2">
                                <label className="text-[10px] text-zinc-400 uppercase tracking-wider mb-1 pl-1 font-bold">Título / Descr.</label>
                            ...
                            <div className="flex flex-col md:col-span-1">
                                ...
                                <input type="date" value={nuevoVideo.fechaPublicacion} onChange={(e) => setNuevoVideo({...nuevoVideo, fechaPublicacion: e.target.value})} className="p-2.5 bg-zinc-900 border border-zinc-700 rounded text-white text-sm focus:outline-none focus:border-zinc-400 [color-scheme:dark]" />
                            </div>
                            <div className="flex flex-col md:col-span-1">
                                ...
                                }} className="p-2.5 bg-zinc-900 border border-zinc-700 rounded text-zinc-300 text-sm focus:outline-none focus:border-zinc-400">
                        ...
                        {/* Filtros de joyas igual que antes */}
                        <div className="bg-zinc-900/30 p-4 rounded-lg border border-zinc-800 mb-6">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <select value={modalFiltroCategoria} onChange={(e) => setModalFiltroCategoria(e.target.value)} className="text-xs bg-zinc-800 border border-zinc-700 text-white rounded p-1">
                                        <option value="">Todas</option><option value="Cadena">Cadena</option><option value="Pulsera">Pulsera</option><option value="Aro">Aro</option><option value="Anillo">Anillo</option>
                                    </select>
                                    <label className="flex items-center gap-1.5 text-xs text-zinc-400 cursor-pointer hover:text-white"><input type="checkbox" checked={modalOcultarAgotados} onChange={(e) => setModalOcultarAgotados(e.target.checked)} className="w-3 h-3 accent-white bg-zinc-800 rounded cursor-pointer" /> Ocultar agotados</label>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-2">
                                {joyasParaElModal.map(joya => (
                                    <button type="button" key={joya.id} onClick={() => toggleJoyaSeleccionada(joya.id)} className={`p-2 text-left text-xs rounded border transition-colors ${joyasSeleccionadas.includes(joya.id) ? 'bg-green-500/20 border-green-500 text-green-400 font-bold' : 'bg-zinc-900 border-zinc-700 text-zinc-400'}`}>
                                        {joyasSeleccionadas.includes(joya.id) && '✓ '} {joya.nombre}
                                    </button>
                                ))}
                            </div>
                        </div>
                        <button onClick={handleRegistrarVideo} className="w-full bg-white text-black hover:bg-zinc-200 font-bold tracking-widest py-3 rounded shadow-lg text-sm">
